// 09_TVBN_旗艦店 正式 renderer（1920 × 1080、JPG）。
// geometry authority（Jamie 裁決）：Jamie Photoshop global geometry ＋ 09 對位圖
//（六元素以 artboard offset X 3030／Y 1734 唯一互證換算為 canvas-local）。
// FSS D－09（09_SPX TVBN_2，1599×1080）僅作 rendering pattern／typography
// behavior／encoder 唯讀 evidence，其 geometry／字級／固定底圖與 09 不同，不得搬入。
// 原設計畫板 global 座標（3144/1975/2102/2212/2359、4023/1970、4681/1781）
// 僅為溯源記錄，不參與 runtime geometry。
// Typography（Jamie 核准）：Photoshop source sizes 90pt／123pt／50pt（Photoshop
// UI 顯示之 pt，非 px），依既有 FSS Photoshop-to-Canvas mapping（×0.75）得
// Canvas 67.5pt／92.25pt／37.5pt。
// Jamie 裁決：
// - 三文字（主標／副標／小字）一律「靠左＋垂直 ink 置中」（沿 07 已驗證
//   left-centered ink pattern）。
// - 副標不沿用 FSS D－09 的 $／% symbol special sizing（65pt symbol font／
//   tokenizer／mixed-run），全字串一律 92.25pt Bold。
// - Medium 2× 於 title 與 small 皆空字串時 early-return；副標直繪。
// - Logo：contain 後「靠左＋垂直置中」（高度先到，dest ≈ 514.5×101，LEFT 有
//   實際視覺意義）；無 Logo runtime intrinsic gate（沿 precedent）。
// - 右上橫式蝦皮購物：contain 後「範圍內靠右＋垂直置中」，runtime intrinsic
//   1119 × 275 gate；與 Logo 共用同一次 resolveLogoVariant 結果同步切換 橘／白。
// - 09 沒有固定底圖（整張填 Workspace background color）、沒有 CTA。
// 文字內容與四色一律來自快速取件 Workspace，不 hardcode FSS 文案與顏色。
// 對位圖（assets/對位/09_TVBN_旗艦店.png）只供開發／人工驗證，不是 runtime dependency。
// 本檔自含必要 helper，不修改、不 import 01～08 renderer。
...
// 字級（Jamie 核准）：Photoshop source 90pt／123pt／50pt ×0.75 →
// Canvas 67.5pt／92.25pt／37.5pt（pt 原樣，不得改 px）。
...
// 正式 layout（canvas-local；Jamie global − offset 3030/1734，與對位圖逐 px 互證）。
// 顏色不在此定義，一律取自 Workspace。
const LAYOUT = Object.freeze({
  logo: Object.freeze({ left: 114, top: 241, width: 640, height: 101 }),
  title: Object.freeze({ left: 114, top: 368, width: 640, height: 82, font: TITLE_FONT }),
  subtitle: Object.freeze({ left: 114, top: 478, width: 830, height: 116, font: SUBTITLE_FONT }),
  small: Object.freeze({ left: 114, top: 625, width: 830, height: 46, font: SMALL_FONT }),
  kv: Object.freeze({ left: 993, top: 236, width: 880, height: 609 }),
  hShopee: Object.freeze({ left: 1651, top: 47, width: 212, height: 52 })
});
...
function registerFontAliases() {
  fontRegistrationPromise ||= Promise.all(
    ...
    throw new Error(`09 正式字型載入失敗：${error.message}`);
  });
...
function assertFontsReady() {
  if (
    ...
    throw new Error("正式字型尚未載入，已停止 09_TVBN_旗艦店 render。");
  }
}

async function ensureFontsReady() {
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 09_TVBN_旗艦店 render。");
  }
...
// --- 固定 assets：店到店 Logo 橘／白＋右上橫式蝦皮購物 橘／白（09 沒有 CTA），
// 載入失敗 fail-closed ---
  ...
  if (!assetPromises.has(assetKey)) {
    const promise = new Promise((resolve, reject) => {
      ...
      image.addEventListener(
        "error",
        () => reject(new Error(`09 正式素材載入失敗：${ASSET_URLS[assetKey].pathname}`)),
        { once: true }
      );
...
// 正確 contain 為高度先到（scale = min(640/1401, 101/275) ≈ 0.36727，
// destination ≈ 514.5 × 101），LEFT 有實際視覺意義。
...
// --- 文字繪製：靠左 ＋ 垂直 ink 置中（沿 07 已驗證 left-centered ink pattern）---
...
// 於 2×（3840 × 2160）暫存 canvas 以原始 1920 × 1080 logical geometry 與
...
function drawMediumLayer(context, title, titleColor, small, smallColor) {
  ...
  if (!mediumContext) {
    throw new Error("無法建立 09 Medium 暫存 Canvas 2D context。");
  }
...
// 09 沒有 CTA。source-over、globalAlpha = 1、無 clipping／masking。

async function renderTvbnFlagship09({ ctx, state }) {
  ...
  if (
    ...
    throw new Error(
      `09 右上蝦皮購物素材必須為 ${H_SHOPEE_SOURCE_WIDTH} × ${H_SHOPEE_SOURCE_HEIGHT}px，已停止 render。`
    );
  }
  ...
  // 5. Bold layer：副標（全字串含 $／% 一律 92.25pt Bold、靠左＋垂直 ink 置中、直繪）
...
registerRenderer("09-tvbn-flagship", renderTvbnFlagship09);

export const TVBN_FLAGSHIP_09_LAYOUT = LAYOUT;
