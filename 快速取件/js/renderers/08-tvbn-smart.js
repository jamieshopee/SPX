// 08_TVBN_智取店 正式 renderer（1080 × 1920、JPG）。
// geometry authority（Jamie 裁決）：Jamie Photoshop global geometry ＋ 08 對位圖
//（六元素以 artboard offset X 1846／Y 1734 唯一互證換算為 canvas-local）。
// FSS D－08（08_SPX TVBN_1，1080×1920）僅作 rendering pattern／encoder 唯讀
// evidence，其 geometry／字級與 08 不同，不得搬入。
// 原設計畫板 global 座標（2066/1965/2092、1961/2202/2345、1886/2587、2674/3562）
// 僅為溯源記錄，不參與 runtime geometry。
// Typography（Jamie 核准）：Photoshop source sizes 90pt／123pt／59pt（Photoshop
// 顯示之 pt，非 px），依既有 FSS Photoshop-to-Canvas mapping（×0.75）得
// Canvas 67.5pt／92.25pt／44.25pt。
// Jamie 裁決：
// - 三文字 ink 水平置中＋垂直置中（沿 05 已驗證 center/center pattern）。
// - 副標不沿用 FSS D－08 的 $／% 較小 symbol font（75pt），全字串一律 92.25pt Bold。
// - Medium 2× 於 title 與 small 皆空字串時 early-return。
// - Logo：contain 後水平置中＋垂直置中（無 runtime intrinsic gate，沿 precedent）。
// - 右下橫式蝦皮購物：contain 後「範圍內靠右＋垂直置中」，runtime intrinsic
//   1119 × 275 gate；與 Logo 共用同一次 resolveLogoVariant 結果同步切換 橘／白。
// - 08 沒有固定底圖（整張填 Workspace background color）、沒有 CTA。
// 文字內容與四色一律來自快速取件 Workspace，不 hardcode FSS 文案與顏色。
// 對位圖（assets/對位/08_TVBN_智取店.png）只供開發／人工驗證，不是 runtime dependency。
// 本檔自含必要 helper，不修改、不 import 01～07 renderer。
...
// 字級（Jamie 核准）：Photoshop source 90pt／123pt／59pt ×0.75 →
// Canvas 67.5pt／92.25pt／44.25pt（pt 原樣，不得改 px）。
const TITLE_FONT = `67.5pt "${MEDIUM_FAMILY}"`;
// 副標（Jamie 裁決）：所有字元（含 $／%）一律 92.25pt Bold，無 symbol font。
const SUBTITLE_FONT = `92.25pt "${BOLD_FAMILY}"`;
const SMALL_FONT = `44.25pt "${MEDIUM_FAMILY}"`;
...
// 正式 layout（canvas-local；Jamie global − offset 1846/1734，與對位圖逐 px 互證）。
// 顏色不在此定義，一律取自 Workspace。
const LAYOUT = Object.freeze({
  logo: Object.freeze({ left: 220, top: 231, width: 640, height: 101 }),
  title: Object.freeze({ left: 220, top: 358, width: 640, height: 84, font: TITLE_FONT }),
  subtitle: Object.freeze({ left: 115, top: 468, width: 850, height: 115, font: SUBTITLE_FONT }),
  small: Object.freeze({ left: 115, top: 611, width: 850, height: 55, font: SMALL_FONT }),
  kv: Object.freeze({ left: 40, top: 853, width: 1000, height: 691 }),
  hShopee: Object.freeze({ left: 828, top: 1828, width: 212, height: 52 })
});

// 右下橫式蝦皮購物 source intrinsic fail-closed 驗證值（正式 asset 為 1119 × 275）。
...
function registerFontAliases() {
  fontRegistrationPromise ||= Promise.all(
    ...
    throw new Error(`08 正式字型載入失敗：${error.message}`);
  });
...
function assertFontsReady() {
  if (
    ...
    throw new Error("正式字型尚未載入，已停止 08_TVBN_智取店 render。");
  }
}

async function ensureFontsReady() {
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 08_TVBN_智取店 render。");
  }
...
// --- 固定 assets：店到店 Logo 橘／白＋右下橫式蝦皮購物 橘／白（08 沒有 CTA），
// 載入失敗 fail-closed ---
  ...
  if (!assetPromises.has(assetKey)) {
    const promise = new Promise((resolve, reject) => {
      ...
      image.addEventListener(
        "error",
        () => reject(new Error(`08 正式素材載入失敗：${ASSET_URLS[assetKey].pathname}`)),
        { once: true }
      );
...
// 右下橫式蝦皮購物用（Jamie 裁決）：contain 縮放後，範圍內靠右＋垂直置中。
...
// --- 文字繪製：ink-box 水平置中 ＋ 垂直置中（沿 05 已驗證 center/center pattern）---
...
// 於 2×（2160 × 3840）暫存 canvas 以原始 1080 × 1920 logical geometry 與
...
function drawMediumLayer(context, title, titleColor, small, smallColor) {
  ...
  if (!mediumContext) {
    throw new Error("無法建立 08 Medium 暫存 Canvas 2D context。");
  }
...
// Medium layer（主標＋小字）→ Bold layer（副標，直繪）→ 右下橫式蝦皮購物。
// 08 沒有 CTA。source-over、globalAlpha = 1、無 clipping／masking。

async function renderTvbnSmart08({ ctx, state }) {
  await ensureFontsReady();

  // variant 只解算一次：同一結果同時決定店到店 Logo 與右下橫式蝦皮購物。
  ...
  // 右下橫式蝦皮購物 intrinsic fail-closed 驗證：正式 asset 必須為 1119 × 275。
  if (
    hShopeeImage.naturalWidth !== H_SHOPEE_SOURCE_WIDTH ||
    hShopeeImage.naturalHeight !== H_SHOPEE_SOURCE_HEIGHT
  ) {
    throw new Error(
      `08 右下蝦皮購物素材必須為 ${H_SHOPEE_SOURCE_WIDTH} × ${H_SHOPEE_SOURCE_HEIGHT}px，已停止 render。`
    );
  }
  ...
  // 3. 店到店 Logo：contain 後水平置中＋垂直置中（Jamie 裁決）
  ...
  // 5. Bold layer：副標（全字串含 $／% 一律 92.25pt Bold、ink 水平＋垂直置中、直繪）
  drawCenteredText(
    ctx,
  ...
  // 6. 右下橫式蝦皮購物：contain 後範圍內靠右＋垂直置中（Jamie 裁決），
  //    與 Logo 同一 variant
  drawContainRightVCenter(ctx, hShopeeImage, LAYOUT.hShopee);
}

registerRenderer("08-tvbn-smart", renderTvbnSmart08);

export const TVBN_SMART_08_LAYOUT = LAYOUT;
