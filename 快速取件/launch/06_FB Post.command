...
# 06_FB Post 專屬 launcher：Finder 雙擊後自動啟動 SPX 本機 HTTP Server（port 4175），
# 並以 Google Chrome 開啟快速取件 Viewer（item=06）。
# 依 SPX 架構（啟動 SPX.command pattern），不依賴 FSS runtime。
# 與 01～05 launcher 共用 port 4175：既有正確 server（Viewer marker 命中）會被安全重用。
...
readonly SPX_ITEM_URL="${SPX_VIEWER_URL}?item=06"
...
if "${SPX_LSOF}" -nP -iTCP:"${SPX_PORT}" -sTCP:LISTEN >/dev/null 2>&1; then
  echo "Port ${SPX_PORT} 已被其他程式占用，無法啟動 06_FB Post Viewer。"
  pause_before_exit
  exit 1
fi
...
if [[ "${spx_server_ready}" != true ]]; then
  echo
  echo "無法啟動 06_FB Post 本機 HTTP Server。請確認連接埠 ${SPX_PORT} 未被其他程式使用。"
  pause_before_exit
  exit 1
fi
...
echo
echo "06_FB Post Viewer 已啟動：${SPX_ITEM_URL}"
